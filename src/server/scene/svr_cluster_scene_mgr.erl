...
% @doc 为指定{活动,分组}创建场景分线 (需要调用 close_scene_line/2 关闭场景分线)
...
% @doc 关场景
close_scene_line(SceneId, LineId) ->
    info({'close_scene_line', SceneId, LineId}).
...
% 为指定{活动,分组}增开场景分线
handle_call({'get_line_info', SvrId, SceneId, LineId}, _From, State) ->
...
% @doc 关闭场景分线
handle_info({'close_scene_line', SceneId, LineId}, State) ->
    case lookup_ets(SceneId) of
...
create_lines_in_clusters(Scene, GroupList) ->
    CreateFun = fun(GroupInfo, {AccLineInfo, AccSvrLine}) ->
        ...
        LinePid = lib_kf:call_to_cluster(ClusterId, svr_scene_agent, start_scene, [Scene, GroupId]),
...
do_create_scene_lines(SceneId, LineIds, #group_info{cluster_id = ClusterId} = GroupInfo) when ClusterId > 0 ->
    ...
    DoCreateLineFun = fun(LineId, {AccEtsScene, PidAcc}) ->
        ...
        case maps:is_key(LineId, LineInfo) of
            false ->
                LinePid = lib_kf:call_to_cluster(ClusterId, svr_scene_agent, start_scene, [Scene, LineId]),
