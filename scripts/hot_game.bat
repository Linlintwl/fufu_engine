...
FOR %%i IN (5) DO erl -noshell -pa ebin -name vt_hot%%i@%IP% -setcookie %COOKIE%_s%%i -eval "case net_adm:ping('%NODE_PREFIX%_s%%i@%IP%') of pong -> rpc:cast('%NODE_PREFIX%_s%%i@%IP%', svr_hot_agent, hot, []); _ -> io:format(\"No nodes!!!\") end, halt(1)"
